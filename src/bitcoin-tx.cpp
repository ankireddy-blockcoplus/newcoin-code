...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " newcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  newcoin-tx [options] <hex-tx> [commands]  Update hex-encoded newcoin transaction\n" +
            "or:     newcoin-tx [options] -create [commands]   Create hex-encoded newcoin transaction\n" +
            "\n";
